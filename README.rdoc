...
+:basic_auth+ accepts string or symbol keys (<tt>'user'</tt>/<tt>'password'</tt>
work the same as <tt>:user</tt>/<tt>:password</tt>), so a hash loaded straight
...
The integration specs (<tt>spec/integration</tt>) need a Solr instance;
run <tt>docker compose up -d</tt> before <tt>bundle exec rspec</tt> or
